...
//   "--un-chinese-typography-root-selector": "",
// };
//
// // 创建一个空的映射对象
// const cssVariableMappings = {};
//
// // 遍历originalCSSVariables并生成映射
// for (const key in originalCSSVariables) {
//   if (originalCSSVariables.hasOwnProperty(key)) {
//     // 去掉前缀并将剩下的key转换为驼峰命名
//     const camelCaseKey = key.replace(/^--un-chinese-typography-/, "").replace(/-([a-z])/g, (match, letter) => letter.toUpperCase());
//     cssVariableMappings[camelCaseKey] = `var(${key})`;
//   }
// }
